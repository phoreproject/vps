...
#
#	Platforms:
#               - Linux Ubuntu 16.04 LTS AND 18.04 LTS ONLY on a Vultr, Hetzner or DigitalOcean VPS
...
function show_help(){
  ...
  echo "Usage example:";
  echo "install.sh (-p|--project) string [(-h|--help)] [(-n|--net) int] [(-c|--count) int] [(-r|--release) string] [(-w|--wipe)] [(-g|--generate)][(-k|--key) string] [(-u|--update)] [(-x|--startnodes)]";
  echo "Options:";
...
function create_mn_configuration() {
  ...
  # check whether masternode private keys are duplicated
  if [ -f tmp.txt ]; then
  ...
  for NUM in $(seq 1 ${count}); do
    ...
    # replace masternode private keys
    if [ -n "${PRIVKEY[${NUM}]}" ]; then
...
function set_permissions() {
  ...
  chmod -R g=u ${MNODE_CONF_BASE} ${MNODE_DATA_BASE} /var/log/sentinel &>> ${SCRIPT_LOGFILE} &>> ${SCRIPT_LOGFILE}
}
